...
UVCLS_INLINE Passwd::Passwd(std::shared_ptr<uv_passwd_t> pwd)
    : passwd{pwd} {}

UVCLS_INLINE std::string Passwd::username() const noexcept {
    return ((passwd && passwd->username) ? passwd->username : "");
}

UVCLS_INLINE decltype(uv_passwd_t::uid) Passwd::uid() const noexcept {
    return (passwd ? passwd->uid : decltype(uv_passwd_t::uid){});
}

UVCLS_INLINE decltype(uv_passwd_t::gid) Passwd::gid() const noexcept {
    return (passwd ? passwd->gid : decltype(uv_passwd_t::gid){});
}

UVCLS_INLINE std::string Passwd::shell() const noexcept {
    return ((passwd && passwd->shell) ? passwd->shell : "");
}

UVCLS_INLINE std::string Passwd::homedir() const noexcept {
    return ((passwd && passwd->homedir) ? passwd->homedir : "");
}

UVCLS_INLINE Passwd::operator bool() const noexcept {
    return static_cast<bool>(passwd);
}

UVCLS_INLINE UtsName::UtsName(std::shared_ptr<uv_utsname_t> utsname)
    : utsname{utsname} {}

UVCLS_INLINE std::string UtsName::sysname() const noexcept {
    return utsname ? utsname->sysname : "";
}

UVCLS_INLINE std::string UtsName::release() const noexcept {
    return utsname ? utsname->release : "";
}

UVCLS_INLINE std::string UtsName::version() const noexcept {
    return utsname ? utsname->version : "";
}

UVCLS_INLINE std::string UtsName::machine() const noexcept {
    return utsname ? utsname->machine : "";
}

UVCLS_INLINE PidType Utilities::OS::pid() noexcept {
    return uv_os_getpid();
}

UVCLS_INLINE PidType Utilities::OS::parent() noexcept {
    return uv_os_getppid();
}

UVCLS_INLINE std::string Utilities::OS::homedir() noexcept {
    return details::tryRead(&uv_os_homedir);
}

UVCLS_INLINE std::string Utilities::OS::tmpdir() noexcept {
    return details::tryRead(&uv_os_tmpdir);
}

UVCLS_INLINE std::string Utilities::OS::env(const std::string &name) noexcept {
    return details::tryRead(&uv_os_getenv, name.c_str());
}

UVCLS_INLINE bool Utilities::OS::env(const std::string &name, const std::string &value) noexcept {
    return (0 == (value.empty() ? uv_os_unsetenv(name.c_str()) : uv_os_setenv(name.c_str(), value.c_str())));
}

UVCLS_INLINE std::string Utilities::OS::hostname() noexcept {
    return details::tryRead(&uv_os_gethostname);
}

UVCLS_INLINE UtsName Utilities::OS::uname() noexcept {
    auto ptr = std::make_shared<uv_utsname_t>();
    uv_os_uname(ptr.get());
    return ptr;
}

UVCLS_INLINE Passwd Utilities::OS::passwd() noexcept {
...
UVCLS_INLINE int Utilities::OS::priority(PidType pid) {
    int prio = 0;
...
UVCLS_INLINE bool Utilities::OS::priority(PidType pid, int prio) {
    return 0 == uv_os_setpriority(pid, prio);
}

UVCLS_INLINE HandleType Utilities::guessHandle(HandleCategory category) noexcept {
    switch(category) {
    case UV_ASYNC:
...
UVCLS_INLINE HandleType Utilities::guessHandle(FileHandle file) noexcept {
    HandleCategory category = uv_guess_handle(file);
    return guessHandle(category);
}

UVCLS_INLINE std::vector<CPUInfo> Utilities::cpuInfo() noexcept {
...
UVCLS_INLINE std::vector<InterfaceAddress> Utilities::interfaceAddresses() noexcept {
...
UVCLS_INLINE std::string Utilities::indexToName(unsigned int index) noexcept {
    return details::tryRead(&uv_if_indextoname, index);
}

UVCLS_INLINE std::string Utilities::indexToIid(unsigned int index) noexcept {
    return details::tryRead(&uv_if_indextoiid, index);
}

UVCLS_INLINE bool Utilities::replaceAllocator(MallocFuncType mallocFunc, ReallocFuncType reallocFunc, CallocFuncType callocFunc, FreeFuncType freeFunc) noexcept {
    return (0 == uv_replace_allocator(mallocFunc, reallocFunc, callocFunc, freeFunc));
}

UVCLS_INLINE std::array<double, 3> Utilities::loadAverage() noexcept {
    std::array<double, 3> avg;
    uv_loadavg(avg.data());
    return avg;
}

UVCLS_INLINE char **Utilities::setupArgs(int argc, char **argv) {
    return uv_setup_args(argc, argv);
}

UVCLS_INLINE std::string Utilities::processTitle() {
...
UVCLS_INLINE bool Utilities::processTitle(const std::string &title) {
    return (0 == uv_set_process_title(title.c_str()));
}

UVCLS_INLINE uint64_t Utilities::totalMemory() noexcept {
    return uv_get_total_memory();
}

UVCLS_INLINE uint64_t Utilities::constrainedMemory() noexcept {
    return uv_get_constrained_memory();
}

UVCLS_INLINE double Utilities::uptime() noexcept {
    double ret;
...
UVCLS_INLINE RUsage Utilities::rusage() noexcept {
    RUsage ru;
    auto err = uv_getrusage(&ru);
    return err ? RUsage{} : ru;
}

UVCLS_INLINE uint64_t Utilities::hrtime() noexcept {
    return uv_hrtime();
}

UVCLS_INLINE std::string Utilities::path() noexcept {
    return details::tryRead(&uv_exepath);
}

UVCLS_INLINE std::string Utilities::cwd() noexcept {
    return details::tryRead(&uv_cwd);
}

UVCLS_INLINE bool Utilities::chdir(const std::string &dir) noexcept {
    return (0 == uv_chdir(dir.data()));
}

UVCLS_INLINE TimeVal64 Utilities::timeOfDay() noexcept {
    uv_timeval64_t ret;
    uv_gettimeofday(&ret);
    return ret;
}

UVCLS_INLINE void Utilities::sleep(unsigned int msec) noexcept {
    int sec;
    int usec;
    sec = msec / 1000;
    usec = (msec % 1000) * 1000;
...
// UVCLS_INLINE unsigned int Utilities::availableParallelism() noexcept {
//     return uv_available_parallelism();
// }

} // namespace uvcls

#endif // UVCLS_UTIL_INCLUDE_H
