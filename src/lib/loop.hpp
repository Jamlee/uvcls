...
UVCLS_INLINE std::shared_ptr<Loop> Loop::create() {
...
UVCLS_INLINE std::shared_ptr<Loop> Loop::create(uv_loop_t *loop) {
...
UVCLS_INLINE bool Loop::alive() const noexcept {
...
UVCLS_INLINE int Loop::descriptor() const noexcept {
    return uv_backend_fd(loop.get());
}

UVCLS_INLINE std::pair<bool, Loop::Time> Loop::timeout() const noexcept {
    auto to = uv_backend_timeout(loop.get());
    return std::make_pair(to == -1, Time{to});
}

// UVCLS_INLINE Loop::Time Loop::idleTime() const noexcept {
//     return Time{uv_metrics_idle_time(loop.get())};
// }

UVCLS_INLINE Loop::Time Loop::now() const noexcept {
    return Time{uv_now(loop.get())};
}

UVCLS_INLINE void Loop::update() const noexcept {
    return uv_update_time(loop.get());
}

UVCLS_INLINE void Loop::fork() noexcept {
...
UVCLS_INLINE void Loop::data(std::shared_ptr<void> uData) {
    userData = std::move(uData);
}

UVCLS_INLINE const uv_loop_t *Loop::raw() const noexcept {
    return loop.get();
}

UVCLS_INLINE uv_loop_t *Loop::raw() noexcept {
